...
/**
 * Fetch remote profile and extract JSON-LD data island
 * @param {string} url - Profile URL to fetch
 * @returns {Promise<Object|null>} Extracted actor or null
 */
async function fetchProfileDataIsland(url) {
  try {
    ...
    // Extract JSON-LD data island
    const match = html.match(/<script\s+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/i)
    if (!match) {
      console.log('   ⚠️ No JSON-LD data island found')
      return null
    }

    const actor = JSON.parse(match[1])
    console.log(`   ✅ Extracted actor: ${actor.id || actor['@id']}`)
    return actor
  } catch (err) {
...
/**
 * Build actor from remote profile or local config
 * Merges remote identity with local AP endpoints
 */
async function buildActorFromProfile() {
  if (!config.profileUrl) return null

  const remoteActor = await fetchProfileDataIsland(config.profileUrl)
  if (!remoteActor) return null

  // Use remote identity but local AP endpoints
  const baseUrl = getBaseUrl()
  const profileUrl = `${baseUrl}/${config.username}`

  return {
    ...remoteActor,
    // Override AP endpoints to point to this server
    ...
    // Keep publicKey from remote or add our own
    publicKey: remoteActor.publicKey || {
      id: `${remoteActor.id?.replace(/#.*$/, '') || profileUrl}#main-key`,
      owner: remoteActor.id || `${profileUrl}#me`,
...
export async function startServer() {
  ...
  // Try to fetch from remote profile URL first, fallback to local config
  if (config.profileUrl) {
    actor = await buildActorFromProfile()
    if (!actor) {
      console.log('⚠️  Could not fetch remote profile, using local config')
      actor = buildActor()
    }
  } else {
    actor = buildActor()
  ...
  server.listen(config.port, () => {
    const actorId = actor.id || `http://localhost:${config.port}/${config.username}#me`
    console.log(`
📦 Fedbox is running!

   Actor:   ${actorId}
   Inbox:   ${actor.inbox}
${config.profileUrl ? `   Profile: ${config.profileUrl} (remote)` : `   Profile: http://localhost:${config.port}/${config.username}`}
